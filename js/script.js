...
function dataProcessing(array) {
  ...
  if (readValueArray.length == 7 && !modbusConfig) {
    ...
    if ((crcReceived1 == crcGenerated1) && (crcReceived2 == crcGenerated2)) {
      ...
      // until chanelBusy is false again (end of this function) no other command can be send
      // this way there is no overlapping of telegrams
...
// the passed array contains the adress byte, coommand byte, 16bits for the start adres, 16bit for the number of registers to read
...
function crc_generator(array) {
  ...
  crc_Bytes = ([parseInt(hexByteSwapped.slice(0, 2), 16), parseInt(hexByteSwapped.slice(2), 16)]); // convert first hex-Byte to Integer and save in Array
  ...
  // attach the two crc Bytes to the pased array
}
